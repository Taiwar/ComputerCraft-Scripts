...
-- Example ME item structure (? copied from RS)
--[[ 
item = {
    amount = 0,
    displayName = 0,
...
local exclusions = {
    ...
    MYSTICAL_AGRICULTURE.."air"..ESSENCE,
    MYSTICAL_AGRICULTURE.."earth"..ESSENCE,
    ...
    MYSTICAL_AGRICULTURE.."cow"..ESSENCE
}
...
local productMappings = {
    ...
    [MYSTICAL_AGRICULTURE.."emerald"..ESSENCE] = {
        [DEFAULT] = {
            recipe = recipes["full"]
        }
    },
...
local function requestEssence(name, stock, recipe)
    ...
    if actual == nil then
        print("No items found in ME system. Skipping.")
        return false
    end
    print("Got "..actual)
    return actual == count
        ...
        for productName, instructions in pairs(mapping) do
            ...
            while enoughStock and wantMoreProduct and enoughSecondary and i < CRAFTING_MAX_ITER do
                ...
                if not fulfilled then
                    print("Requests were not fulfilled by ME. Skipping.")
                    cleanInv()
                    enoughStock = false
                else
